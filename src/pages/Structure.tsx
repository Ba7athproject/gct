...
export default function Structure() {
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            try {
                const module = await import(`../data/mix_transferts_garanties_autres${suffix}.json`);
                setMixData(module.default);
            } catch (e) {
                                            ...
                                            <div>
                                                <div className="flex justify-between items-center text-sm mb-1.5 font-black uppercase tracking-widest gap-2">
                                                    ...
                                                    <span className="text-white">
                                                        {total > 0 ? Math.round((yearData.garanties / total) * 100) : 0}%
                                                    </span>
                                                </div>
                                                <div className="w-full bg-slate-900 rounded-none h-1.5 overflow-hidden">
                                                    <div
                                                        className="bg-blue-500 h-full"
                                                        style={{ width: `${total > 0 ? (yearData.garanties / total) * 100 : 0}%` }}
                                                    ></div>
                                                </div>
                                            </div>
                                            <div>
                                                <div className="flex justify-between items-center text-sm mb-1.5 font-black uppercase tracking-widest gap-2">
                                                    ...
                                                    <span className="text-white">
                                                        {total > 0 ? Math.round((yearData.transferts / total) * 100) : 0}%
                                                    </span>
                                                </div>
                                                <div className="w-full bg-slate-900 rounded-none h-1.5 overflow-hidden">
                                                    <div
                                                        className="bg-emerald-500 h-full"
                                                        style={{ width: `${total > 0 ? (yearData.transferts / total) * 100 : 0}%` }}
                                                    ></div>
                                                </div>
                                            </div>
                                            <div>
                                                <div className="flex justify-between items-center text-sm mb-1.5 font-black uppercase tracking-widest text-slate-300 gap-2">
                                                    ...
                                                    <span>{total > 0 ? Math.round((yearData.autres / total) * 100) : 0}%</span>
                                                </div>
                                                <div className="w-full bg-slate-900 rounded-none h-1.5 overflow-hidden">
                                                    <div
                                                        className="bg-slate-600 h-full"
                                                        style={{ width: `${total > 0 ? (yearData.autres / total) * 100 : 0}%` }}
                                                    ></div>
                                                </div>
                                            </div>
                                        </div>
